    ...
    , log(&Logger::get("InterpreterSelectQuery"))
{
    ...
    query_analyzer = std::make_unique<ExpressionAnalyzer>(
        query_ptr, syntax_analyzer_result, context, {}, required_result_column_names, subquery_depth, !only_analyze);
        ...
        const PrewhereInfoPtr & prewhere_info, const Names & columns_to_remove_after_prewhere)
{
    ...
    if (storage && !storage->getColumns().aliases.empty())
    {
        ...
        if (alias_columns_required)
        {
            ...
            auto additional_source_columns_set
                    = ext::map<NameSet>(additional_source_columns, [] (const auto & it) { return it.name; });

            alias_actions = ExpressionAnalyzer(required_columns_expr_list, syntax_analyzer_result,
                                               context, additional_source_columns).getActions(true);
